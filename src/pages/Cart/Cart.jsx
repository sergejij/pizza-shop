...
const PizzaRow = ({
  photo, headline, price, countInCart, isTraditional, size,
}) => (
  <div className={styles.pizzaRow}>
    ...
    <div className={styles.pizzaChoseBlock}>
      <div className={styles.changeCount}>
        <button type="button" className={styles.changeCountBtn}>
      ...
      <button type="button" className={styles.removePizzaBtn}>
...
const Cart = () => {
  const { pizzasCart: pizzas } = React.useContext(AppContext);
    ...
    <div className={styles.cart}>
      {
          pizzas.length ? (
            <div className={styles.notEmptyCart}>
              ...
              {
                  pizzas.map((pizza) => <PizzaRow key={pizza.id} {...pizza} />)
                }
              <div className={styles.row}>
                <p>
                  Всего пицц:
                  ...
                  <span className={styles.allCount}>
                    {pizzas.reduce((acc, pizza) => acc + pizza.countInCart, 0)}
                    {' '}
                    шт.
                  </span>
                </p>
                <p>
                  ...
                  <span className={styles.allPrice}>
                    {pizzas.reduce((acc, pizza) => acc + pizza.price * pizza.countInCart, 0)}
                    {' '}
                    ₽
                  </span>
                </p>
              ...
              <div className={styles.row}>
                ...
                <button type="button" className={`btn ${styles.orangeBtn}`}>Оплатить сейчас</button>
              </div>
            </div>
